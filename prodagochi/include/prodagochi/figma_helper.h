...
std::shared_ptr<Sprite> &createFromImage(const fs::path &path, const json &data,
  ...
  element = std::make_shared<Sprite>(pos, size, imagePath);
  return element;
}

inline std::shared_ptr<Sprite> createRect(const fs::path &path,
                                          const glm::vec2 windowSize) {
  std::ifstream f(path);
  json data = json::parse(f);
  auto type = data["fills"][0]["type"];
...
inline std::map<std::string, std::shared_ptr<UIElement>>
  ...
  if (fs::is_directory(path)) {
    ...
    for (const auto &entry : fs::directory_iterator(jsonPath)) {
      if (fs::is_regular_file(entry.path())) {
        ...
        ret.insert({entry.path().filename(), rect});
      }
    }
  } else {
